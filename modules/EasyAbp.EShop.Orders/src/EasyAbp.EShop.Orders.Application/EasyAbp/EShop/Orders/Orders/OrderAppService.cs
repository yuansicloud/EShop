        ...
        IOrderAppService
    {
        ...
        protected override async Task<IQueryable<Order>> CreateFilteredQueryAsync(GetOrderListDto input)
        {
            return (await _repository.WithDetailsAsync())
                ...
                .WhereIf(input.MinCompletionDate.HasValue, x => x.CompletionTime.HasValue && x.CompletionTime >= input.MinCompletionDate.Value)
                .WhereIf(input.MaxCompletionDate.HasValue, x => x.CompletionTime.HasValue && x.CompletionTime <= input.MaxCompletionDate.Value)
                .WhereIf(input.MinPaidDate.HasValue, x => x.PaidTime.HasValue && x.PaidTime >= input.MinPaidDate.Value)
                .WhereIf(input.MaxPaidDate.HasValue, x => x.PaidTime.HasValue && x.PaidTime <= input.MaxPaidDate.Value)
